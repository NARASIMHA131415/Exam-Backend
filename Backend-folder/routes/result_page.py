...
def get_result(attempt_id):
    ...
    try:
        ...
        for i, row in enumerate(correct_ans, 1):
            ...
            # Compare using option_label (A, B, C, D) on both sides
            s_compare = str(s_val).strip().upper() if s_val else None
            c_compare = str(row['correct_val']).strip().upper()
            detailed.append({
                "question_number": i,
                "correct_answer": c_compare,
                "student_answer": s_compare,
                "is_correct": s_compare == c_compare
            })
