...
def get_result(attempt_id):
    ...
    try:
        ...
        cursor.execute("SELECT q.question_id, o.option_text as correct_val FROM questions q JOIN options o ON q.question_id = o.question_id WHERE q.exam_id = %s AND o.is_correct = TRUE", (exam_id,))
        ...
        for i, row in enumerate(correct_ans, 1):
            ...
            detailed.append({"question_number": i, "correct_answer": row['correct_val'], "student_answer": s_val, "is_correct": s_val == row['correct_val']})

        return jsonify({"success": True, "data": {**result, "time_taken": time_taken, "answers": detailed, "total_questions": len(correct_ans)}}), 200
...
def get_student_results():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        query = """
            SELECT r.attempt_id, r.score, r.percentage, r.evaluated_at as submitted_at, e.title as exam_title, e.exam_id,
            ...
            JOIN student_attempts sa ON r.attempt_id = sa.attempt_id
            JOIN exams e ON sa.exam_id = e.exam_id
            WHERE sa.user_id = %s ORDER BY r.evaluated_at DESC
        """
...
def get_admin_results():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        query = """
            SELECT r.*, e.title as exam_title, e.exam_code, up.first_name, up.last_name, u.email as student_email,
                   (SELECT COUNT(*) FROM questions WHERE exam_id = e.exam_id) as total_questions,
                   TIMESTAMPDIFF(SECOND, sa.start_time, sa.end_time) as time_taken
            FROM results r
            JOIN student_attempts sa ON r.attempt_id = sa.attempt_id
        ...
        # Enrich results with violations array
        final_results = []
        for r in results:
            # Fetch violations for this attempt
            cursor.execute("SELECT action, timestamp FROM question_attempt_logs WHERE attempt_id = %s AND action LIKE 'VIOLATION%'", (r['attempt_id'],))
            v_logs = cursor.fetchall()
            
            violations = []
            for log in v_logs:
                # The log action is "VIOLATION: Type - Message"
                ...
                violations.append({
                    ...
                    "image": None # Images would be stored in S3/EC2 uploads
                })
